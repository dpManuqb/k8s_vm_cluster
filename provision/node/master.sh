...
do
  sleep 10
  READY=$(ssh $USER@$CLUSTER 'grep LoadbalancerReady /home/vagrant/provision.log | head -1')
done

sudo kubeadm init --control-plane-endpoint $LB_IP:6443 --apiserver-advertise-address $NODE_IP --pod-network-cidr $POD_NETWORK
